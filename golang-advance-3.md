...
* 为了解决上面实现中存在的悬挂指针问题，就需要引入屏障技术来保障数据的一致性。
  ...
  * 写屏障（write barrier），实现过程是通过在 `writePointer(slot, ptr)` 方法调用中加入 `shade` 操作来实现：如果该对象是白色的话，`shade(ptr)`会将对象标记成灰色，这样可以保证强三色不变性，它会保证 ptr 指针指向的对象在赋值给 `*slot` 前不是白色。
    * 优点：实现非常简单，并且也能保证强三色不变性。
